...
def test_structured_tool_from_function_with_args_param() -> None:
    """Test that StructuredTool.from_function filters out v__args sentinel field.

    When a function has a parameter named 'args', Pydantic's v1 validated function
    creates a sentinel field named 'v__args'. This should be filtered out from the
    schema to prevent spurious fields from appearing in the tool description.
    """

    def run_command(working_dir: str, args: list[str], timeout: int = 60) -> str:
        """Run a command."""
        return f"{working_dir}: {args}"

    st = StructuredTool.from_function(run_command)
    schema = st.args_schema.model_json_schema()

    # v__args should be filtered out
    assert "v__args" not in schema["properties"], (
        "v__args sentinel field should be filtered from schema"
    )
    # The legitimate 'args' parameter should be present
    assert "args" in schema["properties"], (
        "The 'args' parameter should be present in schema"
    )
