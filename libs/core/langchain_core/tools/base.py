...
def create_schema_from_function(
    ...
    *,
    ...
    if existing_params and existing_params[0] in {"self", "cls"} and in_class:
        default_filter_args = [existing_params[0], *list(FILTERED_ARGS)]
    else:
        default_filter_args = list(FILTERED_ARGS)

    # Collect injected args to filter if include_injected=False
    injected_args_to_filter = []
    for existing_param in existing_params:
        if _is_injected_arg_type(sig.parameters[existing_param].annotation):
            injected_args_to_filter.append(existing_param)

    if filter_args:
        filter_args_ = list(filter_args)
        # Even when filter_args is provided, respect include_injected=False
        # by adding injected args to the filter list
        if not include_injected:
            for injected_arg in injected_args_to_filter:
                if injected_arg not in filter_args_:
                    filter_args_.append(injected_arg)
    else:
        filter_args_ = default_filter_args
        if not include_injected:
            filter_args_.extend(injected_args_to_filter)
    ...
    for field in get_fields(inferred_model):
        ...
        if field == "v__args":  # Internal pydantic field when function has 'args' param
            continue
